...
const getParsedValue = (scope: Construct, key: string, value: string): string => {
    if (value.indexOf("ssm:") > -1) {
        ...
        if (groups && groups.length > 0) {
            ...
            const latestStringToken = new aws.dataAwsSsmParameter.DataAwsSsmParameter(
                scope, "ssm-" + key, configSSM as aws.dataAwsSsmParameter.DataAwsSsmParameterConfig
            )
...
export const UpdateEnvironmentVariable = (scope: Construct, env: IEnvironmentVars): IEnvironmentVars => {
